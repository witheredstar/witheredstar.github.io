function getRootPath_dc() {
  ...
  if (webName == "") {
    ...
    return window.location.protocol + '//' + window.location.host;//(+ webName)强制获取根目录链接
  }
}// 获得根目录链接
var cc=getRootPath_dc();// 赋予cc获取的值
var cs=cc+"/css/images/2.png";// 合成绝对链接
var stop,staticx;
var img=new Image();
img.src=cs;// 应用链接
...
function getRandom(option)
  {
	  ...
	  switch(option)
	    {
			case 'x':ret=Math.random()*window.innerWidth;
			break;
			case 'y':ret=Math.random()*window.innerHeight;
			break;
			case 's':ret=Math.random();
			break;
			case 'r':ret=Math.random()*6;
			break;
            ...
            ret=function(x,y)
			  {return y+random;};
			break;
			case 'fnr':random=Math.random()*0.03;
			ret=function(r)
			  {return r+random;};
			  break;
		}
	  ...
	  for(var i=0;i<16;i++)
	    {
			...
			sakura=new Sakura(randomX,randomY,randomS,randomR,{x:randomFnx,y:randomFny,r:randomFnR});sakura.draw(cxt);sakuraList.push(sakura);
			}
